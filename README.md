...
To train a novel model the input data should be an RNA-RNA interactome specifying all RRIs sites within a organism (Cell line). Cherri makes use of replicate data by checking if a RRI site can be found in all replicates within a overlap threshold. This is how Cherri builds the set of trusted RRIs. In the evaluation mode the interaction positions are reformatted in the same way as the trusted RRIs. Than in both cases Cherri's core method can be run to generate the training or feature set which will be in the train mode used for model selection and in the evaluation mode used for the classification of the instances.
...
For the test call please downloade the [Cherri_models_data](https://zenodo.org/record/6533932#.Ynu--FxBwUE).
```
cherri eval -i1 /vol/scratch/Cherri/test_data/evaluate/test_evalueat_rris.cvs -g human -l human -o ./ -n test_eval -c 150 -st on
-m ./Cherri_models_data/Model_with_graph_features/PARIS_human/model/full_PARIS_human_context_150.model -mp ./Cherri_models_data/Model_with_graph_features/PARIS_human/feature_files/training_data_PARIS_human_context_150.npz
```
...
Please select the Chira RRI output files as input for Cherri train. You should specify the path to the folders containing all replicats. If you want to filer for interactions, where Chira also already found a hybrid within its analysis you should enable the hybridization with the Chira workflow and set for Cherri the filter_hybrid parameter as 'on'. Some of the possible interaction maid be missed this way, but it will also filter out potential false positive interactions. 
...
Cherri is build as a modular tool calling individual scrips doing specific tasks. If you would like to only perform one step of the Cherri pipeline you can do this by calling the individual scrips. A short description of this scripts is given in the following.
